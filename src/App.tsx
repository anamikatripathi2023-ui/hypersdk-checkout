...
// ─── TOKEN VAULT (simulated tokenization) ────────────────────────────────────
const TokenVault = {
  tokenize: (cardData) => {
...
// ─── NETWORK ICONS ─────────────────────────────────────────────────────────
const NetworkBadge = ({ network, size = 28 }) => {
  const cfg = {
    visa:       { bg: "#1A1F71", text: "#fff",    label: "VISA",       font: "bold italic" },
    mastercard: { bg: "#EB001B", text: "#fff",    label: "MC",         font: "bold" },
    rupay:      { bg: "#1a6b3c", text: "#fff",    label: "RuPay",      font: "bold" },
    amex:       { bg: "#007BC1", text: "#fff",    label: "AMEX",       font: "bold" },
    unknown:    { bg: "#27272A", text: "#71717A", label: "CARD",       font: "normal" },
  };
  ...
  return (
    <div style={{ width: size * 1.6, height: size, background: c.bg, borderRadius: 4, display: "flex", alignItems: "center", justifyContent: "center" }}>
      <span style={{ color: c.text, fontSize: size * 0.38, fontWeight: 800, fontStyle: c.font.includes("italic") ? "italic" : "normal", letterSpacing: "-0.02em" }}>{c.label}</span>
    </div>
  );
};
...
const CardPreview = ({ form }) => {
  ...
  const gradients = { visa: "135deg, #1a237e, #1565c0", mastercard: "135deg, #8B0000, #c62828", rupay: "135deg, #1a3c1a, #2e7d32", amex: "135deg, #01579b, #0288d1", unknown: "135deg, #18181b, #27272a" };
  return (
    <div style={{ width: "100%", maxWidth: 300, height: 170, borderRadius: 14, background: `linear-gradient(${gradients[net]})`, padding: "18px 22px", position: "relative", overflow: "hidden", boxShadow: "0 20px 60px rgba(0,0,0,0.5)", flexShrink: 0 }}>
      ...
      <div style={{ fontFamily: "'Courier New', monospace", fontSize: 17, color: "#fff", letterSpacing: "0.18em", marginBottom: 16, opacity: 0.95 }}>
        {(form.number || "•••• •••• •••• ••••").padEnd(19, "•").slice(0,19).replace(/[^•\s\d]/g, "•")}
      </div>
...
const inputStyle = (error, focused) => ({
  width: "100%", padding: "11px 13px", background: focused ? "#18181B" : "#111113",
...
// ─── PROCESSING OVERLAY ───────────────────────────────────────────────────────
const ProcessingState = ({ state, method, amount }) => {
  const msgs = { processing: ["Encrypting card data...", "Connecting to bank...", "Authenticating...", "Confirming payment..."] };
  const [msgIdx, setMsgIdx] = useState(0);
  useEffect(() => {
    if (state !== "processing") return;
    const t = setInterval(() => setMsgIdx(i => (i + 1) % msgs.processing.length), 700);
  ...
  if (state === "processing") return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "48px 20px", gap: 20 }}>
      ...
      <div style={{ textAlign: "center" }}>
        ...
        <div style={{ fontSize: 13, color: "#71717A", minHeight: 20, transition: "opacity 0.3s" }}>{msgs.processing[msgIdx]}</div>
      </div>
    </div>
  );
  ...
  if (state === "success") return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "48px 20px", gap: 16 }}>
      ...
      <div style={{ textAlign: "center" }}>
        ...
        <div style={{ fontSize: 14, color: "#71717A" }}>₹{amount} paid via {method}</div>
        <div style={{ fontSize: 11, color: "#52525B", marginTop: 8, fontFamily: "'Courier New', monospace" }}>
          TXN#{Math.random().toString(36).toUpperCase().slice(2, 12)}
        </div>
      </div>
      <div style={{ marginTop: 8, padding: "8px 16px", background: "#10b98120", border: "1px solid #10b98140", borderRadius: 6, fontSize: 12, color: "#10b981" }}>
        Token saved for future 1-click payments
      </div>
    </div>
  );
...
// ─── MAIN SDK COMPONENT ──────────────────────────────────────────────────────
function CheckoutSheet({ isOpen, onClose, amount, merchant, onSuccess, onFailure }) {
  const [tab, setTab] = useState("saved");
  const [flow, setFlow] = useState("idle"); // idle | processing | success | failed
  const [cardForm, setCardForm] = useState({ number: "", expiry: "", cvv: "", name: "", saveCard: true });
  const [cardErrors, setCardErrors] = useState({});
  const [focusedField, setFocusedField] = useState(null);
  const [upiVpa, setUpiVpa] = useState("");
  const [upiValid, setUpiValid] = useState(null);
  const [selectedWallet, setSelectedWallet] = useState(null);
  const [selectedSaved, setSelectedSaved] = useState(TokenVault.savedCards[0]);
  ...
  const reset = () => {
    ...
    setSelectedWallet(null); setSelectedSaved(TokenVault.savedCards[0]); setSavedCvv("");
    setTab("saved");
  };
  ...
  const simulate = useCallback((methodLabel) => {
    setFlow("processing");
    setTimeout(() => {
      const ok = Math.random() > 0.2;
      setFlow(ok ? "success" : "failed");
      if (ok) onSuccess?.(methodLabel);
      else onFailure?.(methodLabel);
    }, 2800);
  }, [onSuccess, onFailure]);
  ...
  const payCard = () => {
    ...
    const { token } = TokenVault.tokenize(cardForm);
    simulate(`Card •••• ${cardForm.number.slice(-4)}`);
  };

  const paySaved = () => {
    if (savedCvv.length < 3) return;
    simulate(`Saved Card •••• ${selectedSaved.last4}`);
  };

  const payUpi = () => { if (!upiValid) return; simulate(`UPI ${upiVpa}`); };
  const payWallet = () => { if (!selectedWallet) return; simulate(WALLETS.find(w => w.id === selectedWallet)?.name); };
  ...
  const methodLabel = flow === "success" || flow === "failed" ? null : null;
            ...
            <div style={{ display: "flex", borderBottom: "1px solid #18181B", overflowX: "auto", flexShrink: 0 }}>
              {tabs.map(t => (
                <button key={t.id} onClick={() => setTab(t.id)} style={{
                  flex: "1 0 auto", padding: "12px 8px", background: "transparent",
                  border: "none", borderBottom: tab === t.id ? "2px solid #6366f1" : "2px solid transparent",
                  color: tab === t.id ? "#6366f1" : "#71717A",
            ...
            <div style={{ overflowY: "auto", flex: 1, padding: "20px" }}>
              ...
              {tab === "saved" && (
                <div>
                  ...
                  {TokenVault.savedCards.map(card => (
                    <div key={card.token} onClick={() => setSelectedSaved(card)} style={{
                      padding: "12px 14px", marginBottom: 8, borderRadius: 10,
                      border: `1px solid ${selectedSaved?.token === card.token ? "#6366f1" : "#27272A"}`,
                      background: selectedSaved?.token === card.token ? "#6366f120" : "#111113",
                      cursor: "pointer", display: "flex", alignItems: "center", gap: 12, transition: "all 0.15s",
                    }}>
                      <div style={{ width: 10, height: 10, borderRadius: "50%", border: `2px solid ${selectedSaved?.token === card.token ? "#6366f1" : "#52525B"}`, background: selectedSaved?.token === card.token ? "#6366f1" : "transparent", transition: "all 0.15s", flexShrink: 0 }} />
                  ...
                  {selectedSaved && (
                    <div style={{ marginTop: 16 }}>
                      <Field label="CVV" error={savedCvv.length > 0 && savedCvv.length < 3 ? "Invalid CVV" : ""} hint="3-digit code on back">
                        <input
                          type="password" maxLength={selectedSaved.network === "amex" ? 4 : 3}
                          value={savedCvv} onChange={e => setSavedCvv(e.target.value.replace(/\D/g,""))}
                          placeholder="•••" style={{ ...inputStyle(savedCvv.length > 0 && savedCvv.length < 3, focusedField === "scvv"), width: 100 }}
                          onFocus={() => setFocusedField("scvv")} onBlur={() => setFocusedField(null)}
                        />
                      </Field>
                      <button onClick={paySaved} style={{
                        width: "100%", padding: "14px", background: savedCvv.length >= 3 ? "linear-gradient(135deg, #6366f1, #4f46e5)" : "#27272A",
                        border: "none", borderRadius: 10, color: "#fff", fontSize: 15, fontWeight: 700,
                        cursor: savedCvv.length >= 3 ? "pointer" : "not-allowed", fontFamily: "inherit",
                ...
                <div>
                  ...
                  <Field label="CARD NUMBER" error={cardErrors.number}>
                    <div style={{ position: "relative" }}>
                      <input
                        ...
                        style={{ ...inputStyle(cardErrors.number, focusedField === "number"), fontFamily: "'Courier New', monospace", letterSpacing: "0.15em", paddingRight: 50 }}
                        inputMode="numeric"
                      />
                      <div style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)" }}>
                        <NetworkBadge network={detectNetwork(cardForm.number)} size={20} />
                      </div>
                    </div>
                  </Field>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                    <Field label="EXPIRY" error={cardErrors.expiry}>
                      <input
                        ...
                        placeholder="MM/YY" style={{ ...inputStyle(cardErrors.expiry, focusedField === "expiry"), fontFamily: "'Courier New', monospace", letterSpacing: "0.1em" }}
                        inputMode="numeric"
                      />
                    </Field>
                    <Field label="CVV" error={cardErrors.cvv}>
                      <input
                        ...
                        placeholder="•••" style={{ ...inputStyle(cardErrors.cvv, focusedField === "cvv"), fontFamily: "'Courier New', monospace", letterSpacing: "0.2em" }}
                      />
                    </Field>
                  </div>
                  <Field label="NAME ON CARD" error={cardErrors.name}>
                    <input
                      ...
                      style={{ ...inputStyle(cardErrors.name, focusedField === "name"), textTransform: "uppercase", letterSpacing: "0.05em" }}
                    />
                  </Field>
              ...
              {tab === "upi" && (
                <div>
                  ...
                  <Field label="UPI ID / VPA" hint={upiValid === false ? "Format: name@bankname" : upiValid === true ? "✓ VPA looks valid" : "e.g. yourname@okhdfc"}>
                    <div style={{ position: "relative" }}>
                      <input
                        value={upiVpa}
                        onChange={e => handleUpiChange(e.target.value)}
                        onFocus={() => setFocusedField("upi")} onBlur={() => setFocusedField(null)}
                        placeholder="name@bankname"
                        style={{ ...inputStyle(upiValid === false && upiVpa.length > 5, focusedField === "upi"), paddingRight: 36 }}
                      />
                      {upiValid !== null && (
                        <div style={{ position: "absolute", right: 12, top: "50%", transform: "translateY(-50%)", color: upiValid ? "#10b981" : "#f87171", fontSize: 16 }}>
                          {upiValid ? "✓" : "✕"}
                        </div>
                      )}
                    </div>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 16 }}>
                    {WALLETS.map(w => (
                      <div key={w.id} onClick={() => setSelectedWallet(w.id)} style={{
                        ...
                        border: `1px solid ${selectedWallet === w.id ? w.color : "#27272A"}`,
                        background: selectedWallet === w.id ? w.color + "18" : "#111113",
                        cursor: "pointer", display: "flex", alignItems: "center", gap: 10, transition: "all 0.15s",
                      }}>
                        <div style={{ width: 36, height: 36, borderRadius: 8, background: w.color, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, fontWeight: 800, color: "#fff", flexShrink: 0 }}>
                          {w.icon}
                        </div>
                        <div>
          ...
          <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center" }}>
            ...
            {(flow === "success" || flow === "failed") && (
              <div style={{ padding: "0 20px 20px", display: "flex", gap: 10 }}>
                {flow === "failed" && (
                  <button onClick={() => setFlow("idle")} style={{ flex: 1, padding: "13px", background: "transparent", border: "1px solid #3F3F46", borderRadius: 10, color: "#E4E4E7", fontSize: 14, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>
                    Try Again
                  </button>
                )}
  ...
  return (
    <div style={{ fontFamily: "'DM Sans', 'Segoe UI', sans-serif", background: "#F5F5F0", minHeight: "100vh", maxWidth: "100vw", overflow: "hidden" }}>
      ...
      <div style={{ padding: "16px" }}>
          ...
          {PRODUCTS.map(p => {
            ...
            return (
              <div key={p.id} className="prod-card" style={{ background: "#fff", borderRadius: 12, padding: "14px", display: "flex", alignItems: "center", gap: 14, border: `1.5px solid ${inCart ? "#6366f1" : "#E5E7EB"}`, cursor: "pointer" }} onClick={() => toggleCart(p)}>
                ...
                <div className="add-btn" style={{ width: 30, height: 30, borderRadius: "50%", background: inCart ? "#6366f1" : "#111", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: 18, fontWeight: 700, flexShrink: 0, transition: "all 0.15s" }}>
                  {inCart ? "✓" : "+"}
                </div>
              </div>
            );
        ...
        {/* Checkout Button — SDK Entry Point */}
        <button
          className="checkout-btn"
          onClick={() => { setLastResult(null); setSheetOpen(true); }}
          disabled={cart.length === 0}
          style={{
            width: "100%", padding: "16px", borderRadius: 12,
            background: cart.length > 0 ? "linear-gradient(135deg, #6366f1, #4f46e5)" : "#E5E7EB",
            border: "none", color: cart.length > 0 ? "#fff" : "#9CA3AF",
            fontSize: 16, fontWeight: 800, cursor: cart.length > 0 ? "pointer" : "not-allowed",
            fontFamily: "inherit", display: "flex", alignItems: "center", justifyContent: "center", gap: 8,
            boxShadow: cart.length > 0 ? "0 8px 30px rgba(99,102,241,0.35)" : "none",
          }}
        >
